...
def verify_api_key(
    ...
    *,
    ...
    scoped_token = B58Encoder().decode(signer.unsign(key, max_age=ttl))
...
def generate_api_key(*, scope: AuthScopes, secret: str) -> ApiKey:
    ...
    token = B58Encoder().encode(token_bytes(16))
    signed_key = signer.sign(
        B58Encoder().encode(get_scope_id(scope) + API_KEY_SCOPE_SEPARATOR.encode() + token),
    )
